...
interface Channel {
  ...
  members_formatted: string; has_photo: boolean; photo_url?: string;
}
...
function Avatar({ ch }: { ch: Channel }) {
  ...
  if (ch.has_photo && ch.photo_url && !err) {
    return <img src={ch.photo_url} alt={ch.title} onError={() => setErr(true)}
      style={{ width: 48, height: 48, borderRadius: '50%', objectFit: 'cover', flexShrink: 0 }} />;
  }
  return (
    <div style={{ width: 48, height: 48, borderRadius: '50%', background: bg,
...
/* ── Карточка ───────────────────────────────────────────────────────────── */
function ChannelCard({ ch, onSync, onDelete, busy }: {
  ch: Channel; onSync: () => void; onDelete: () => void; busy: boolean;
}) {
  return (
    <div style={{ background: '#2E2F33', borderRadius: 22, padding: '14px 16px',
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <p style={{ fontSize: 11, color: '#7D7D7D', marginTop: 3 }}>
          Подписчиков: <span style={{ color: 'rgba(255,255,255,0.5)' }}>{ch.members_formatted}</span>
        </p>
      </div>
...
export default function ChannelsPage() {
  ...
  const handleDelete = (id: number) => {
    const tg = window.Telegram?.WebApp;
    tg?.showPopup({ message: 'Удалить канал? Бот выйдет из него.',
      ...
      setChannels(p => p.filter(c => c.id !== id)); setActionId(null);
    });
  };
      ...
      <main style={{ flex: 1, padding: '16px 16px 120px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {loading
          ...
          : channels.length === 0
            ...
            : channels.map(ch => (
                <ChannelCard key={ch.id} ch={ch} busy={actionId === ch.id}
                  onSync={() => handleSync(ch.id)} onDelete={() => handleDelete(ch.id)} />
              ))}
      </main>
