  ...
  const handlePublish = async () => {
    ...
    try {
      const payload = {
        ...
        boost_channels:    store.boostChannels,        // ← каналы для буста
  ...
  if (sent) return (
    <div style={{ minHeight: '100vh', background: '#0B0B0B', display: 'flex', flexDirection: 'column',
      ...
      <p style={{ fontSize: 14, color: '#7D7D7D', lineHeight: 1.6, marginBottom: 32 }}>
        Детали розыгрыша отправлены в бот. Нажмите «Принять» там, чтобы опубликовать.
      </p>
      <GradientButton onClick={() => { router.replace('/'); window.Telegram?.WebApp?.BackButton?.hide(); }}>
        На главную
      </GradientButton>
    </div>
  );
    ...
    <div style={{ minHeight: '100vh', background: '#0B0B0B', display: 'flex', flexDirection: 'column' }}>
      ...
      <main style={{ flex: 1, padding: '20px 16px 120px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ background: '#2E2F33', borderRadius: 22, overflow: 'hidden' }}>
          {[
            ...
            store.useBoosts && store.boostChannels.length > 0
              ? ['Каналы для буста', `${store.boostChannels.length} шт.`]
              : null,
          ...
          ].filter((item): item is [string, string] => item !== null).map(([label, value], i, arr) => (
        ...
        <p style={{ fontSize: 12, color: '#424141', paddingLeft: 4, lineHeight: 1.5 }}>
          После нажатия «Опубликовать» детали розыгрыша придут в бота — там нужно подтвердить запуск.
        </p>
      </main>
