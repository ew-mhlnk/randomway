...
export default function Step3Page() {
  ...
  const handleNext = () => {
    if (store.publishChannels.length === 0) {
      return window.Telegram?.WebApp.showAlert("Выберите хотя бы один канал для публикации розыгрыша");
    }
  ...
  return (
    <main className="p-4 pb-24 flex flex-col gap-6 animate-in fade-in slide-in-from-right-4 duration-300">
      <div>
        <h2 className="text-xl font-bold text-(--text-primary)">Публикация</h2>
        <p className="text-sm text-(--text-secondary) mt-1">
          В каких каналах опубликовать этот розыгрыш?
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col gap-3">
          {channels.map(ch => {
            const isSelected = store.publishChannels.includes(ch.id);
            return (
              <div 
                key={ch.id}
                onClick={() => { haptic?.selectionChanged(); store.toggleChannel('publishChannels', ch.id); }}
