...
import { Users, Gift, Activity, LogOut } from 'lucide-react';
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-white/5 text-gray-400">
                <tr>
                  ...
                  <th className="p-4 font-medium">Кол-во Призов</th>
                  <th className="p-4 font-medium">Статус</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5">
                {stats.giveaways.map((gw: any) => (
                  <tr 
                    key={gw.id} 
                    onClick={() => router.push(`/admin/giveaways/${gw.id}`)}
                    className="hover:bg-white/10 transition-colors cursor-pointer"
                  >
                    <td className="p-4 text-gray-400">#{gw.id}</td>
                    <td className="p-4 font-medium">{gw.title}</td>
                    <td className="p-4">{gw.winners} 🏆</td>
                    <td className="p-4">
                      <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${
                        gw.status === 'active' ? 'bg-green-500/20 text-green-400' : 
                        gw.status === 'completed' ? 'bg-gray-500/20 text-gray-400' : 'bg-yellow-500/20 text-yellow-400'
                      }`}>
                        {gw.status}
                      </span>
                    </td>
