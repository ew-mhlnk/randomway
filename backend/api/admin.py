...
async def get_global_stats(
    ...
    return {
        ...
        "giveaways": [
            {"id": g.id, "title": g.title, "status": g.status, "winners": g.winners_count}
            for g in all_gw
        ]
    }
...
async def manual_set_winners(
    giveaway_id: int,
    payload: SetWinnersRequest,
    is_admin: bool = Depends(verify_admin_token),
    db: AsyncSession = Depends(get_db)
):
    giveaway = await db.scalar(select(Giveaway).where(Giveaway.id == giveaway_id))
    ...
    if payload.winner_ids:
        await db.execute(
            ...
            .where(
                ...
                Participant.user_id.in_(payload.winner_ids)
            )
