...
class GiveawayService:
    ...
    async def send_confirmation_to_bot(self, db: AsyncSession, bot: Bot, user_id: int, giveaway_id: int):
        ...
        confirm_text = (
            ...
            "⚠️ Розыгрыш будет опубликован сразу после подтверждения!"
        )
            ...
            async with AsyncSessionLocal() as db:
                ...
                async def check_one(p):
                    ...
                    checks = await asyncio.gather(*[
                        _check_member_safe(bot, ch.telegram_id, p.user_id) for ch in sponsor_channels])
                    return p, all(checks)

                for i in range(0, len(participants), 50):
                    results = await asyncio.gather(*[check_one(p) for p in participants[i:i+50]])
                    for p, ok in results:
                        if ok: valid.append(p)
                        else: p.is_active = False; db.add(p)
                    await db.commit()
                ...
                for p in valid:
                    if p.is_winner:
                        ...
                        t = 1 + min(getattr(p, 'boost_count', 0), 10) + p.invite_count
                        pool.extend([p.user_id] * t)

                winners = set(pre_w)
                while len(winners) < giveaway.winners_count and pool:
                    c = rng.choice(pool); winners.add(c)
                    pool = [x for x in pool if x != c]

                for p in valid:
                    p.is_winner = p.user_id in winners; db.add(p)
                giveaway.status = "completed"
                await db.commit()

                if giveaway.result_channel_ids:
                    wd = await participant_repo.get_winners_with_users(db, giveaway_id)
                    wt = "\n".join([f"🏆 {u.first_name}" + (f" (@{u.username})" if u.username else "") for _, u in wd])
                    text = f"🎉 <b>Итоги розыгрыша «{giveaway.title}»!</b>\n\n{wt}\n\n<i>Честных: {len(valid)}</i>"
                    for ch in await channel_repo.get_by_ids(db, giveaway.result_channel_ids):
                        try: await bot.send_message(chat_id=ch.telegram_id, text=text); await asyncio.sleep(0.5)
                        except Exception as e: logging.error(f"Итоги {ch.title}: {e}")
        finally:
            await bot.session.close()
    ...
    async def draw_additional_winners(self, db: AsyncSession, bot: Bot, giveaway_id: int, count: int, user_id: int):
        ...
        rng = random.SystemRandom()
        g = await giveaway_repo.get_by_id(db, giveaway_id)
        if not g or g.creator_id != user_id: raise HTTPException(status_code=403, detail="Нет прав")
        if g.status != "completed": raise HTTPException(status_code=400, detail="Не завершён")
        ...
        for p in parts:
            if p.is_active and not p.is_winner:
                t = 1 + min(getattr(p,'boost_count',0),10) + p.invite_count
                pool.extend([p.user_id] * t); avail[p.user_id] = p
        ...
        while len(nw) < count and pool:
            c = rng.choice(pool); nw.add(c); pool = [x for x in pool if x != c]

        for wid in nw: avail[wid].is_winner = True; db.add(avail[wid])
        ...
        if g.result_channel_ids:
            ...
            for ch in chs:
                try: await bot.send_message(chat_id=ch.telegram_id, text=post); await asyncio.sleep(0.5)
                except Exception as e: logging.error(f"Доп. победители {ch.title}: {e}")
