...
class ParticipantService:
    async def join_giveaway(self, db: AsyncSession, bot: Bot, giveaway_id: int, user_id: int, ref_code: str | None = None) -> dict:
        """Изолированная логика проверки подписок и регистрации участника"""
        
        # 1. Запрашиваем данные через репозитории
        ...
        # 2. Логика проверки подписок в Telegram
        missing_channels =[]
        ...
        # Если есть неподписки, прерываем выполнение и возвращаем список
        if missing_channels:
            return {"status": "missing_subscriptions", "channels": missing_channels}

        # 3. Подписки пройдены. Записываем в базу.
        ...
        if not participant:
            ...
            if ref_code:
                ...
                # TODO: Dispatch Celery task: notify_referrer(ref_code)

        # 4. Формируем чистый ответ для Роутера
        return {
            "status": "success",
            "giveaway": {
