...
def _request_chat_kb() -> ReplyKeyboardMarkup:
    """Нативная клавиатура выбора канала/группы с проставлением прав"""
...
async def _save_chat(chat_id, owner_id: int, bot: Bot) -> tuple[bool, str, int]:
    ...
    async with AsyncSessionLocal() as db:
        ...
        if existing:
            ...
            existing.owner_id = owner_id # Обновляем оунера на всякий случай
            await db.commit()
...
async def cmd_add_channel_deep_link(message: Message, state: FSMContext):
    await state.set_state(ChannelStates.waiting_for_channel)
    await message.answer(
        "💬 Пришлите username канала в формате <b>@durov</b> или перешлите сообщение "
...
# ── Обработка нативного выбора канала (KeyboardButtonRequestChat) ─────────────
...
async def on_chat_shared(message: Message, bot: Bot, state: FSMContext):
    ...
    try:
        ...
        await message.answer("❌ Ошибка при добавлении. Убедитесь, что у бота есть нужные права.")
...
async def process_manual(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        if member.status != "administrator":
            await message.answer("❌ Бот ещё не администратор в этом канале.")
            return
        ...
        await message.answer("❌ Не удалось найти канал/группу или проверить права.")


# ── Бот автоматически добавлен как админ (через настройки канала) ─────────────
...
async def bot_added_as_admin(event: ChatMemberUpdated):
    ...
    try:
        ...
        await event.bot.send_message(
            chat_id=user_id,
            text=f"✅ {kind} <b>{title}</b> добавлен!\nТеперь вы можете использовать его.",
            reply_markup=_back_kb(),
        )
    except Exception as e:
